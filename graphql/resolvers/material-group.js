...
module.exports = {
  ...
  materialGroupByKey: async args => {
    try {
      const materialGroup = await MaterialGroup.findOne({
        materialGroupKey: args.materialGroupKey
      });
      return transformMaterialGroup(materialGroup);
    } catch (err) {
      throw err;
  ...
  createMaterialGroupCopy: async args => {
    try {
      const materialGroupRef = await MaterialGroup.findById(args.id).populate(
        "auxMaterials"
      );
      //Generate AuxMaterials copies
      const auxMaterials = await Promise.all(
        materialGroupRef.auxMaterials.map(async auxMaterial => {
  ...
  updateMaterialGroupFromDB: async args => {
    try {
      ...
      while (i > 0) {
        let auxMaterial = await auxMaterial.findById(
          materialGroup.auxMaterials[i]
        );
        const material = await Material.findOne({
          materialKey: auxMaterial.materialKey
        });
